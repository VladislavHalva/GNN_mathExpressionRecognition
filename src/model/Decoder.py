...
class Decoder(nn.Module):
    def __init__(self, hidden_size, embed_size, vocab_size, end_node_token_id):
        ...
        self.gcn1 = GCNDecLayer(hidden_size, embed_size, is_first=True)
        self.gcn2 = GCNDecLayer(hidden_size, embed_size, is_first=False)
    ...
    def forward(self, data):
        ...
        # note : use torch.exp() to get categorical probability from log_softmax output
        return data

    def generate_output_graph(self, data):
        out_x = torch.zeros(0, dtype=torch.float)
        tgt_edge_index = torch.zeros(0, dtype=torch.long)
        tgt_edge_type = torch.zeros(0, dtype=torch.long)
    ...
    def generate_output_subtree(self, data, x, edge_index, edge_type, x_root_id, x_root_parent_id,
        ...
        new_node = torch.zeros((1, self.embed_size), dtype=torch.float)
        x = torch.cat([x, new_node], dim=0)

        # add self-loop edge
        cc_edge = torch.tensor([[new_node_id], [new_node_id]], dtype=torch.long)
        edge_index = torch.cat([edge_index, cc_edge], dim=1)
        edge_type = torch.cat([edge_type, torch.tensor([SltEdgeTypes.CURRENT_CURRENT], dtype=torch.long)])
        # add parent-child edge
        if x_root_id:
            pc_edge = torch.tensor([[x_root_id], [new_node_id]], dtype=torch.long)
            edge_index = torch.cat([edge_index, pc_edge], dim=1)
            edge_type = torch.cat([edge_type, torch.tensor([SltEdgeTypes.PARENT_CHILD], dtype=torch.long)])
        # add grandparent-grandchild edge
        if x_root_parent_id:
            gg_edge = torch.tensor([[x_root_parent_id], [new_node_id]], dtype=torch.long)
            edge_index = torch.cat([edge_index, gg_edge], dim=1)
            edge_type = torch.cat([edge_type, torch.tensor([SltEdgeTypes.GRANDPARENT_GRANDCHILD], dtype=torch.long)])
        # add leftBrother-rightBrother edge
        if x_root_last_child_id:
            bb_edge = torch.tensor([[x_root_last_child_id], [new_node_id]], dtype=torch.long)
            edge_index = torch.cat([edge_index, bb_edge], dim=1)
            edge_type = torch.cat([edge_type, torch.tensor([SltEdgeTypes.LEFTBROTHER_RIGHTBROTHER], dtype=torch.long)])

        # run through graph convolutional layers
        x = self.gcn1(data.x, x, edge_index, edge_type)
        x = self.gcn2(data.x, x, edge_index, edge_type)

        # decode new node token and check if end leaf node
        new_node_features = x[new_node_id]
        new_node_features = self.lin_z_out(new_node_features)
        new_node_features = F.softmax(new_node_features, dim=0)
        _, max_id = new_node_features.max(dim=0)
