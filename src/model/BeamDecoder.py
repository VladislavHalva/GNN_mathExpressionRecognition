...
class BeamDecoder(nn.Module):
    def __init__(self, device, f_size, in_size, h_size, emb_size, vocab_size, end_node_token_id, tokenizer, emb_dropout_p, att_dropout_p, beam_width=3):
    ...
    def forward(self, x, x_batch, tgt_y=None, tgt_edge_index=None, tgt_edge_type=None, tgt_y_batch=None):
        if self.training:
            ...
            y, y_batch, y_beam, y_edge_index, y_edge_type = self.gen_graph(x, x_batch)
            ...
            beam_results = self.split_beams_to_separate_trees(
                ...
                calc_joint_prob=True, sort_by='tokens')
    ...
    def gen_graph(self, x, x_batch):
        ...
        y_init, y, y_batch, y_beam, y_eindex, y_etype, _, _ = self.gen_subtree(x, x_batch, y_init, y, y_batch, y_beam, y_eindex, y_etype, gen_tree, pa_ids, gp_ids, ls_ids)
    ...
    def gen_subtree(self, x, x_batch, y_init, y, y_batch, y_beam, y_eindex, y_etype, gen_tree, pa_ids, gp_ids, ls_ids):
        ...
        for i, i_beam in enumerate(beam_idx_unfinisned):
            ...
            if pa_ids[i_beam] is not None:
                y_eindex, y_etype = self.create_edge(y_eindex, y_etype, pa_ids[i_beam], y_new_idx[i], SltEdgeTypes.PARENT_CHILD)
            if gp_ids[i_beam] is not None:
                y_eindex, y_etype = self.create_edge(y_eindex, y_etype, gp_ids[i_beam], y_new_idx[i], SltEdgeTypes.GRANDPARENT_GRANDCHILD)
            if ls_ids[i_beam] is not None:
                y_eindex, y_etype = self.create_edge(y_eindex, y_etype, ls_ids[i_beam], y_new_idx[i], SltEdgeTypes.LEFTBROTHER_RIGHTBROTHER)
        # process with GCNs
        ...
        if not any(gen_subtree):
            ...
            while any(gen_subtree):
                ...
                y_init, y, y_batch, y_beam, y_eindex, y_etype, subl_ls_ids, last_leaf = \
                    self.gen_subtree(x, x_batch, y_init, y, y_batch, y_beam, y_eindex, y_etype, gen_subtree, y_new_idx_per_batch, pa_ids, subl_ls_ids)
                # stop batch items tree generation for those batch items, whose trees reached maximal nodes count
                beam_items_nodes_counts = torch.unique(y_beam, return_counts=True, sorted=True)[1]
                beam_items_nodes_limit_reached = (beam_items_nodes_counts > self.max_output_graph_size)
                # update for which batch items the subtree generation shall continue
                gen_subtree = [True if gen_subtree_i and not last_leaf[i] and not beam_items_nodes_limit_reached[i] else False for i, gen_subtree_i in enumerate(gen_subtree)]
            return y_init, y, y_batch, y_beam, y_eindex, y_etype, y_new_idx_per_batch, this_leaf

    def split_beams_to_separate_trees(self, y_score, y, y_batch, y_beam, y_edge_index, y_edge_type, y_edge_rel_score, calc_joint_prob=True, sort_by='total'):
        ...
        for beam_id in beam_ids:
            ...
            y_edge_index_beam_ids = [i for i, src in enumerate(y_edge_index[0]) if src in y_beam_ids and y_edge_index[1][i] in y_beam_ids]
        ...
        if calc_joint_prob:
            for beam in beams:
                ...
                # print(f"{beam['y'].shape[0]} : {beam['joint_total_certainty']}")

            if sort_by not in ['total', 'tokens', 'edges']:
                sort_by = 'total'
            sort_by = 'joint_' + sort_by + '_certainty'
            # beams = sorted(beams, key=itemgetter(sort_by), reverse=True)
